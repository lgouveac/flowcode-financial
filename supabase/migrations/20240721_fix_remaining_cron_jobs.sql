...
SELECT cron.schedule(
  'invoke-notifications-every-minute-fixed',
  '* * * * *', -- every minute
  $$
...
-- Unschedule the old check-billing-notifications job if it exists
SELECT cron.unschedule('check-billing-notifications');

-- Fix the check_billing_notifications function to not rely on app.settings.service_key
    ...
    FOR interval_record IN SELECT * FROM email_notification_intervals LOOP
        ...
        LOOP
            ...
            IF due_date_this_month - interval_record.days_before * INTERVAL '1 day' = current_date_var THEN
                ...
                -- Send email using the edge function with direct token authentication
                ...
                RAISE LOG 'Email request sent to % (%). Check function logs for details.',
                    billing_record.name,
...
-- Update the trigger_notifications function to not rely on app.settings.service_key
CREATE OR REPLACE FUNCTION public.trigger_notifications()
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
  ...
  -- Only proceed if current time is very close to notification time (within 2 minutes)
  IF NOT (current_time_var BETWEEN settings_notification_time - INTERVAL '2 minutes' AND settings_notification_time + INTERVAL '2 minutes') THEN
    RAISE LOG 'Notification time not matched in trigger. Skipping email check.';
    RETURN json_build_object(
      'success', true, 
      'message', 'Notification time not matched. Skipping.', 
      'current_time', current_time_var, 
      'notification_time', settings_notification_time
    );
  END IF;
  
  -- Execute the notification function
  PERFORM check_billing_notifications();
