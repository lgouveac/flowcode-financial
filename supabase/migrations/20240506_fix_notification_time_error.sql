...
-- Fix the check_billing_notifications function to handle empty or NULL notification_time values
...
BEGIN
    ...
    SELECT coalesce(notification_time, '18:35:00'::time) INTO notification_time
    ...
    IF notification_time IS NULL THEN
        ...
        notification_time := '18:35:00'::time;
    ...
    IF NOT (current_time_var BETWEEN notification_time - INTERVAL '1 minute' AND notification_time + INTERVAL '1 minute') THEN
        RAISE LOG 'Notification time not matched. Current: %, Expected: % (±1 minute). Skipping email sending.',
            current_time_var, notification_time;
        RETURN;
    END IF;
