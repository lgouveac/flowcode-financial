...
-- Fix the employee notification system to use consistent authentication

-- First, drop the existing employee notification cron jobs to avoid duplicates
SELECT cron.unschedule('invoke-employee-notifications-daily');
SELECT cron.unschedule('invoke-employee-notifications-daily-fixed');
SELECT cron.unschedule('invoke-employee-notifications-daily-direct');

-- Create a new cron job with the proper schedule (9 AM daily) and direct token authentication
...
-- Update the check_employee_notifications function to remove any reliance on app.settings.service_key
