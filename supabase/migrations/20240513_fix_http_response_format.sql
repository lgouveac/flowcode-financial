...
-- Atualiza a função check_billing_notifications para utilizar aliases e lidar corretamente com a resposta HTTP
    ...
    FOR interval_record IN SELECT * FROM email_notification_intervals LOOP
        ...
        LOOP
            ...
            IF due_date_this_month - interval_record.days_before * INTERVAL '1 day' = current_date_var THEN
                ...
                -- IMPORTANTE: Usar uma abordagem diferente para acessar a resposta HTTP
                -- Primeiro, obter a resposta HTTP completa como jsonb
                http_response := (
                ...
                -- Em seguida, extrair os campos relevantes da resposta
                response_record := jsonb_populate_record(
                    null::record, 
                    COALESCE(http_response, '{}'::jsonb)
                );

                -- Log da resposta completa para depuração
                RAISE LOG 'HTTP Response: %', http_response;
                
                -- Log da tentativa de envio de email com detalhes
                RAISE LOG 'Email sending attempt to % (%). Response: %',
                    billing_record.name,
