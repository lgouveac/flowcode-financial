...
-- Fix the check_billing_notifications function to correctly handle the HTTP response
    ...
    FOR interval_record IN SELECT * FROM email_notification_intervals LOOP
        ...
        LOOP
            ...
            IF due_date_this_month - interval_record.days_before * INTERVAL '1 day' = current_date_var THEN
                ...
                -- CRITICAL FIX: Get the entire HTTP response as jsonb 
                SELECT content
                INTO response_json
                FROM net.http_post(
                ...
                RAISE LOG 'HTTP Response JSON: %', response_json;
                
                -- Log email sending with response status from jsonb
                RAISE LOG 'Email sending attempt to % (%). Response status: %',
                    billing_record.name,
                    billing_record.email,
                    CASE 
                        WHEN response_json->>'status' IS NOT NULL THEN response_json->>'status'
                        ELSE 'unknown status'
                    END;
            ELSE
