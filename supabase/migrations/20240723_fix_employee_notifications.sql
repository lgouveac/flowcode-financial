...
-- Update the cron job to use direct token authentication instead of relying on app.settings.service_key
...
-- Unschedule the old cron jobs if they exist
...
-- Create a new cron job that uses direct token authentication
SELECT cron.schedule(
  'invoke-employee-notifications-daily-direct',
  '* * * * *', -- For testing: run every minute. Change to '0 9 * * *' (9 AM daily) in production
  $$
