  ...
  const handleTestNotification = async () => {
    try {
      ...
      const {
        data: monthlyValues,
        error: monthlyError
      } = await supabase.from('employee_monthly_values').select('*, employees!inner(*)').eq('month', currentMonth);
      if (monthlyError) {
      ...
      if (data.totalSent > 0) {
        ...
        const noValuesMessage = monthlyValues?.length === 0 ? "Não há valores mensais cadastrados para o mês atual." : "Não há funcionários ativos com valores mensais para o mês atual.";
        toast({
